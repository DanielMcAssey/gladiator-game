...
public class HealthPowerup extends Powerup {
    ...
    public HealthPowerup(ContentManager contentManager, World newWorld, Vector2 startPosition, String newName) {
        ...
        contentManager.loadTexture("textures/powerups/health.png", "powerup_health");

        contentManager.waitForLoad();

        Texture powerupHealth = contentManager.getTexture("powerup_health");
        animation = new ActionAnimation(powerupHealth, new Vector2(0, 0), 23, 0.05f, true);
        stateTime = 0f;

        Vector2 textureSize = new Vector2(powerupHealth.getWidth() / 23, 16);
    ...
    @Override
    public void dispose() {
        contentManager.disposeTexture("powerup_health");
