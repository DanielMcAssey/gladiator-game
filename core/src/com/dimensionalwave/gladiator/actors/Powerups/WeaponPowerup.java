...
public class WeaponPowerup extends Powerup {
    ...
    public WeaponPowerup(ContentManager contentManager, World newWorld, Vector2 startPosition, String newName) {
        ...
        contentManager.loadTexture("textures/powerups/sword.png", "powerup_sword");

        contentManager.waitForLoad();

        Texture powerupSword = contentManager.getTexture("powerup_sword");
        animation = new ActionAnimation(powerupSword, new Vector2(0, 0), 10, 0.05f, true);
        stateTime = 0f;

        Vector2 textureSize = new Vector2(powerupSword.getWidth() / 10, 16);
    ...
    @Override
    public void dispose() {
        contentManager.disposeTexture("powerup_sword");
